...
//            builder.claim("roles", user.getAuthorities());

            builder.claim("authorities", user.getAuthorities());

            System.out.println("**JW Util builder" + builder.getClaims());

            builder.expirationTime(generateExpirationDate());
            JWTClaimsSet claimsSet = builder.build();

            System.out.println("**JW Util claimset" + claimsSet.getClaims());


            SignedJWT signedJWT = new SignedJWT(new JWSHeader(JWSAlgorithm.HS256), claimsSet);

            System.out.println("**JW Util signed" + signedJWT.getJWTClaimsSet());
